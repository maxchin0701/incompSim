...
for(i in 1:100){
  ...
  for(j in 1:length(h)){
    ...
    for(l in indID){
      ...
      #get sex
      SDLGeno <- paste0(curIndHaps[1,68],
                        curIndHaps[2,68])
      if(SDLGeno=="10"){
    ...
    #set colnames
    colnames(indGenoDf) <- c("pop",paste0("locus",1:24))
    
    #### CALCULATE FIS ####
    curPopFis <- basic.stats(data=indGenoDf,diploid=T)$Fis[,1]
...
#Set column names
colnames(df)[c(1,3,4)] = c("replicate","sex","pop")
