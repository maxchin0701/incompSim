...
prefCieff <- as.numeric(arrArgs[3])/10
if(prefCieff <= 0.6){
...
my_func = function(increment_value,prefCieff){
  sexsel = read.delim(paste(sexsel_file), sep = "\t")
  sexsel$Selection[3:5] = paste0("s=",prefCieff+increment_value,
                                 ",1-((1-(Courter_perMal/100))*s)")
  write.table(sexsel, paste(sexsel_file), sep = '\t', quote = FALSE, row.names = FALSE)
}

my_func(increment_value,prefCieff)
